...
def evaluate_model(model, eval_loader, device):
    ...
    up_interval, down_interval = ci

    return accuracy, center, up_interval, down_interval
...
def main(config_file):
    ...
    for epoch in range(num_epochs):  # Start with a larger number of epochs
        ...
        accuracy, center, up_interval, down_interval = evaluate_model(model, dev_loader, device)
        print(f"Epoch {epoch+1}, Loss: {train_loss:.4f}, Accuracy: {accuracy:.4f}, CI: {up_interval:.4f}, {down_interval:.4f}")
    ...
    accuracy, _, up_interval, down_interval = evaluate_model(model, eval_loader, device)
    print(f"Accuracy for {label_col}: {accuracy:.4f}, with CI: {up_interval:.4f}, {down_interval:.4f}")
